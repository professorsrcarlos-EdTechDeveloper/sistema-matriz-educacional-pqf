...
async function tentarReconhecerCodigoAluno(){
  ...
  const av = await buscarAvaliacaoPorCodigoOnline(codigo);
  if(av){
    const serieAuto = String(av.serie || serieDaTurmaTreino(av.turma) || serieDoCodigo(av.codigoProva || "") || "");
    if(box){
      box.innerHTML = `<strong>Código de avaliação reconhecido:</strong> ${av.codigo}<br>${av.aluno_nome_completo ? "Aluno: "+av.aluno_nome_completo+"<br>" : ""}Turma: ${av.turma || "não informada"}<br>Prova: ${av.codigoProva || "não informada"}<br><small>Escola, série e prova serão preenchidas automaticamente ao entrar.</small>`;
      box.style.display = "block";
    }
    if(av.escola) document.getElementById("nomeEscola").value = av.escola;
    if(serieAuto) document.getElementById("serieAluno").value = serieAuto;
  ...
  if(prova){
    const serieAuto = String(prova.serie || serieDoCodigo(prova.codigo || codigo) || "");
    if(box){
      box.innerHTML = `<strong>Código de prova reconhecido:</strong> ${prova.codigo || codigo}<br>Componente: ${prova.componente || "não informado"}<br><small>Para teste livre, a escola será preenchida automaticamente se ficar vazia.</small>`;
      box.style.display = "block";
    }
    document.getElementById("codigoProva").value = prova.codigo || codigo;
    if(serieAuto) document.getElementById("serieAluno").value = serieAuto;
    if(!document.getElementById("nomeEscola").value.trim()) document.getElementById("nomeEscola").value = "EMEF Pedro de Queiroz Ferreira";
    return;
  }
  if(box){
...
function serieDaTurmaTreino(turma){
  const t=String(turma||"");
...
async function iniciar(){
  ...
  let provaDireta=null;
  if(!avaliacaoCriada && codigoAlunoOuAvaliacao){
    provaDireta=await obterProvaPorCodigoAsync(codigoAlunoOuAvaliacao);
  }
